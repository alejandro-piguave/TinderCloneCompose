...
val dataModule = module {
    ...
    //Profile
    single { ProfileRemoteDataSourceImpl(get(), get()) }
    single<ProfileGenerator> { ProfileGeneratorImpl(get()) }
    single<ProfileRepository> { ProfileRepositoryImpl(get()) }

    //Messages
    single { MessageRemoteDataSourceImpl(get(), get()) }
    single<MessageRepository> { MessageRepositoryImpl(get()) }

    //Matches
    single { MatchRemoteDataSourceImpl(get(), get(), get(), get()) }
    single<MatchRepository> { MatchRepositoryImpl(get()) }

    //Auth
    single { AuthLocalDataSourceImpl(get()) }
