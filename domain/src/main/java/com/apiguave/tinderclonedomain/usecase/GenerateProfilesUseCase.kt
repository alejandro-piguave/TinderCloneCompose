    ...
    suspend operator fun invoke(amount: Int): Result<Unit> {
        return Result.runCatching {
            (0 until amount).forEach { _ ->
                ...
                profileRepository.createProfile(profile.id, profile.name, profile.birthdate, profile.bio, profile.gender, profile.orientation, remotePictures.map { it.filename })
            }
        }
    }
}